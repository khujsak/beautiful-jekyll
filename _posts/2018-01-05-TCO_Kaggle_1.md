...
Note that the space is still in reduced coordinates.  In order to interpret the information in our vornoi cell and compare it to tesselations of other structures (with different lattice basis vectors), we will need to convert to a common set of coordinates: real space.  To do this, we'll simply multiply by the original crystal lattice vectors, as described above.
